...
###   dclrcoincore.org and dclrcoin.org and compares them.
...
WORKINGDIR="/tmp/dclrcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://dclrcoincore.org"
HOST2="https://dclrcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="dclrcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'dclrcoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "dclrcoin.org failed to provide signature file, but dclrcoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "dclrcoin.org and dclrcoincore.org signature files were not equal?"
