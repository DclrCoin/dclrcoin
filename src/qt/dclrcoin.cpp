...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. dclrcoin_de.qm (shortcut "de" needs to be defined in dclrcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. dclrcoin_de_DE.qm (shortcut "de_DE" needs to be defined in dclrcoin.qrc)
...
static const char* qt_argv = "dclrcoin-qt";
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // dclrcoin: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(dclrcoin);
    Q_INIT_RESOURCE(dclrcoin_locale);
    ...
    /// 6. Determine availability of data directory and parse dclrcoin.conf
    ...
    // dclrcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
