...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " dclrcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  dclrcoin-tx [options] <hex-tx> [commands]  Update hex-encoded dclrcoin transaction\n" +
            "or:     dclrcoin-tx [options] -create [commands]   Create hex-encoded dclrcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded dclrcoin transaction
