...
The headless daemon `dclrcoind` has the JSON-RPC API enabled by default, the GUI
`dclrcoin-qt` has it disabled by default. This can be changed with the `-server`
...
- **Securing remote network access:** You may optionally allow other
  ...
  `dclrcoind -help` for more information about these settings and other
